...
async def result(job_id: str) -> dict[str, Any]:
    ...
    return {
        ...
        "code": job["code"],
        "history": job["history"],
        "chart_url": f"/chart/{job_id}" if job.get("chart_path") else None,
    }


@app.get("/chart/{job_id}")
async def chart(job_id: str) -> FileResponse:
    job = JOBS.get(job_id)
    if job is None or not job.get("chart_path"):
        raise HTTPException(status_code=404, detail="chart not found")
    chart_path = Path(job["chart_path"])
